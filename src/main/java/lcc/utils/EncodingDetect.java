...
public class EncodingDetect {
	 ...
	 * 通过文件BOM判断字符编码
	 * @param content 文件内容
	 * @return 如果文件没有BOM信息，返回null；否则返回字符编码
	 */
	public static String getEncodeByBOM(byte[] content) {
		if (content.length > 3 && content[0] == 0X00 && content[1] == 0X00 && content[2] == (byte)0XFE && content[3] == (byte)0XFF) {
			return "UTF-32BE";
		} else if (content.length > 3 && content[0] == (byte)0XFF && content[1] == (byte)0XFE && content[2] == 0X00 && content[3] == 0X00) {
			return "UTF-32LE";
		} else if (content.length > 2 && content[0] == (byte)0XEF && content[1] == (byte)0XBB && content[2] == (byte)0XBF) {
			return "UTF-8";
		} else if (content.length > 1 && content[0] == (byte)0XFE && content[1] == (byte)0XFF) {
			return "UTF-16BE";
		} else if (content.length > 1 && content[0] == (byte)0XFF && content[1] == (byte)0XFE) {
			return "UTF-16LE";
		}
		return null;
	}
	
	public static byte[] getBOMBytes(String encode) {
		if ("UTF-32BE".equalsIgnoreCase(encode)) {
			return new byte[]{0X00,0X00,(byte)0XFE,(byte)0XFF};
		} else if ("UTF-32LE".equalsIgnoreCase(encode)) {
			return new byte[]{(byte)0XFF,(byte)0XFE,0X00,0X00};
		} else if ("UTF-8".equalsIgnoreCase(encode)) {
			return new byte[]{(byte)0XEF,(byte)0XBB,(byte)0XBF};
		} else if ("UTF-16BE".equalsIgnoreCase(encode)) {
			return new byte[]{(byte)0XFE,(byte)0XFF};
		} else if ("UTF-16LE".equalsIgnoreCase(encode)) {
